...
export const CombinedFamilyExplorer = () => {
  ...
  if (isLoading) {
    return <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-6 text-slate-500 dark:text-slate-400">Loading combined family data...</div>;
  }
  if (error) {
    return <div className="rounded-xl border border-red-500/40 bg-red-500/10 p-6 text-red-300">{error}</div>;
  }
  if (!detail || !hasPeople) {
    return <div className="rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-6 text-slate-500 dark:text-slate-400">Combined family data unavailable.</div>;
  }
